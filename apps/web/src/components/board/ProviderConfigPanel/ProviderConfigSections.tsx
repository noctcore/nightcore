...
function McpStatusChip({ status }: { status: string }) {
  ...
  return (
    <span className={`font-mono text-3xs-plus font-semibold ${tone}`}>
      {status}
    </span>
  );
}
    ...
    <section className="rounded-[10px] border border-border bg-white/[0.02]">
      <div className="flex items-center gap-2 border-b border-border px-3.5 py-2.5">
        ...
        {section.status === 'supported' && count !== undefined && (
          <span className="rounded-md border border-border bg-white/[0.04] px-1.5 py-0.5 font-mono text-3xs text-muted-foreground">
            {count}
          </span>
        )}
      </div>
  ...
  return (
    <li className="py-1.5">
      <div className="flex items-center gap-2">
        <span className="font-mono text-xs-flat text-foreground">{name}</span>
        {meta}
      </div>
...
export function Extras({ snapshot }: { snapshot: ProviderConfigSnapshot }) {
  ...
  const permissionModeRow = (
    <div className="flex items-center justify-between gap-3">
      <span className="text-xs-flat text-muted-foreground">Permission mode</span>
  ...
  if (snapshot.extrasStatus === 'unavailable') {
    return (
      <div className="flex flex-col gap-2">
        {permissionModeRow}
        <p className="text-xs-flat text-destructive">
    ...
    <div className="flex flex-col gap-2">
      {entries.map(([label, value]) => (
        <div key={label} className="flex items-center justify-between gap-3">
          <span className="text-xs-flat text-muted-foreground">{label}</span>
