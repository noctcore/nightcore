...
export function IssueSyncNotice({ task }: IssueSyncNoticeProps) {
  ...
  return (
    <div className="flex items-start gap-2 rounded-nc border border-warning/40 bg-warning/[0.08] px-3 py-2.5">
      <AlertIcon size={15} className="mt-0.5 shrink-0 text-warning" />
      <p className="flex-1 text-xs-plus leading-snug text-warning/90">{task.issueSyncError}</p>
      <IconButton
        label="Dismiss the issue-sync notice"
        onClick={dismiss}
        className="-mr-1 -mt-0.5 hover:bg-warning/[0.15]"
      >
        <CloseIcon size={13} />
      </IconButton>
    </div>
