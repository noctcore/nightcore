...
test('renders the finding title, inert body, and grounded location', async () => {
  ...
  await expect
    ...
    .toBeInTheDocument();
  await expect.element(screen.getByText('src/a.ts:12')).toBeInTheDocument();
  await expect
    .element(screen.getByText(/rejected write is swallowed/i))
    .toBeInTheDocument();
});
...
test('renders the suggested fix as a syntax-highlighted code block', async () => {
  ...
  // its own element, proving the suggested fix is wired through CodeBlock.
