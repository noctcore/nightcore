...
export interface PrReviewViewModel {
  hasProject: boolean;
  projectName: string | null;
  /** Left rail (persistent PR list) + selection + aggregates. */
  list: {
    prs: PrSummary[];
    prsLoading: boolean;
  ...
  /** Resizable persisted split between PR list rail and the workspace panel. */
  panel: ResizablePanel;
  /** Right panel workspace data (summary + lifecycle + assembled review section). */
  workspace: {
  ...
  /** Push-fix + status remediation gates (the fix arc side effects). */
  fix: {
