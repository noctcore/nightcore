//! Shared Issue-Triage plumbing: the `gh` deadline, the defense-in-depth size caps,
//! `cap_text`, and the tolerant `gh api graphql` deserialization envelope reused by
//! the `list`/`detail`/`diff`/`post` seams.
