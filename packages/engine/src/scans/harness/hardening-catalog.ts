/**
 * Hardening-catalog module producers for {@link hardeningReference}, re-exported from
 * `reference.ts` (packages/engine/src/scans/harness/reference.ts) so the public import
 * path is unchanged. Split into its own file to keep both under the
 * `engine-file-size-ratchet` 400-line cap: the static two-layer harness knowledge base
 * (`HARNESS_REFERENCE`) stays in reference.ts, while this file holds the
 * profile-conditional hardening-module guidance (docs/research/2026-07-02-hardening-module-catalog.md).
 */
