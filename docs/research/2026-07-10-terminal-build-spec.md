...
| 7 | Renderer | **WebGL default-on except macOS**, with `onContextLoss` auto-fallback to DOM, and a Settings toggle that overrides either way. *Amended 2026-07-30 (#407):* the original call was DOM-default everywhere "while xtermjs#5816 is open". That issue — WebGL corruption reported from a Tauri app — is still open and was re-confirmed on shipping macOS 15.7 (2026-05), and xterm 6 removed the canvas renderer, so a corrupted context has only DOM to fall back to. macOS therefore keeps DOM; Windows (WebView2) and Linux get the GPU renderer by default. `Settings.terminalWebglEnabled` became tri-state (`null` ⇒ platform default) so an explicit opt-out is never flipped; the resolution lives in `apps/web/src/lib/platform.ts` (`resolveTerminalWebgl`). Flip the guard when the upstream fix ships in a released addon. *Re-checked 2026-07-31 (#430) — still DOM on macOS:* fix PR xtermjs#5883 **merged** upstream 2026-05-21 (the "unmerged" note above was true when written and is now stale), but it lands entirely in `@xterm/addon-webgl`, whose newest **stable** release is `0.19.0` (2025-12-22) — the version pinned here, which predates the merge. The fix is published only in `0.20.0-beta.219`+ prereleases, which peer-require a beta `@xterm/xterm`. The flip-back trigger is a **stable** `@xterm/addon-webgl` ≥ 0.20.0 carrying #5883, not #5816 being closed. |
